...
def encryptfile(passwordentry: Entry, filedirectory: str) -> None:
    ...
    if password == 'password':
        ...
        if  messagebox.askyesnocancel("askyesnocancel", "Try again?"):
            pass
        else:
            return None
        ...
        with open(filedirectory, 'rb') as encfile:
            with open(filedirectory[:-4], 'wb') as newfile:
                ...
                if cipher[0] == 1:
                    ...
                    messagebox.showerror("showerror","decrypted with v1")
                elif cipher[0] == 2:
                    content = decryptfile2(cipher, password)
                    printdebug('decrypted with v2')
                    messagebox.showerror("showerror","decrypted with v2")
                else:
                    printdebug('unknown encryption version')
                    messagebox.showerror("showerror","unknown encryption version")
                    askwin = Tk()
                    askwin.title(text(10) + ' - ' + text(1))
                    a1 = Label(askwin, text=text(11) + '\n' + text(12))
                    options = [text(13), 'v1']
                ...
                messagebox.showinfo("showinfo","Decrpted file")
    except:
...
def donothing():
   ...
   mystring = StringVar()
   mystring= '''File Encryption Tool is an encryption - decryption tool written in Python which uses RSA encryption technique. The RSA algorithm is for sending any kind of data
over a public connection. In simple terms, user can use the tool to encrypt their files whatsoever it may be,
   ...
   labelU.configure(wraplength=300, bg='blue', fg='white',activebackground='coral')
   labelU.pack()
...
menubar = Menu(root)
helpmenu = Menu(menubar, tearoff=0, activebackground='coral', bg='green')
helpmenu.add_command(label="About File Encryption", command=donothing)
...
a1 = Button(root, text="       LOAD FILE      ", command=lambda: selectfile(a2), width=16,bg='cyan', fg='black', activebackground='coral')
a2 = Label(root, text=filename,width=16,)
b = Entry(root,text='ENTER KEY', width=40, show='*',fg='red')
c1 =Button(root, text="       ENCRYPT      ", command=lambda: encryptfile(b, fileselected), width=16,bg='cyan', fg='black', activebackground='coral')
c2 = Button(root,text="       DENCRYPT      ", command=lambda: decryptfile(b, fileselected), width=16,bg='cyan', fg='black', activebackground='coral')
d1 = Button(root, text="       Send RSA      ", command=lambda: sendrsa(b, fileselected), width=16,bg='cyan', fg='black', activebackground='coral')
d2 = Button(root, text="       Receive RSA      ", command=lambda: receiversa(b, fileselected), width=16,bg='cyan', fg='black', activebackground='coral')
e1 = Button(root, text="       Copy Public Key      ", command=lambda: copypublic(root), width=16,bg='cyan', fg='black', activebackground='coral')
e2 = Button(root, text="       About      ", command=donothing, width=16,bg='cyan', fg='black', activebackground='coral')
a1.grid(row=0, column=0,padx=20, pady=15)
a2.grid(row=0, column=1)
b.grid(row=1, column=0, columnspan=2)
c1.grid(row=2, column=0,pady=15)
c2.grid(row=2, column=1,pady=15)
d1.grid(row=3, column=0,pady=15)
d2.grid(row=3, column=1,pady=15)
e1.grid(row=4, column=0,pady=15)
e2.grid(row=4, column=1,pady=15)
root.mainloop()
